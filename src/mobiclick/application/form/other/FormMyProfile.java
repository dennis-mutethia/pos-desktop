    ...
    private void saveUserDetail() {
        ...
        try (Connection conn = DBConnect.getConnection()) {
            String query = "UPDATE users "
                    + "SET username=?, name=?, phone=?, password=SHA2(?,256), updated_at=NOW(), updated_by=? "
                    + "WHERE id=?";
            ...
            try (PreparedStatement pstmt = conn.prepareStatement(query)) {
                ...
                if ("".equals(username) || "".equals(password)) {
                    Notifications.getInstance().show(Notifications.Type.WARNING, Notifications.Location.TOP_RIGHT, 5000, "Missing UserName and/or Password");
                } else {
